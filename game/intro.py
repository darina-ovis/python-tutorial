...
player = Actor(current_player_image)
...
def update () :
    global is_moving_to_right , ground , sky_color_blue , sky_color_red , sky_color_green , from_dark_to_light , current_player_image
...
def flip_image ( is_moving_to_right: bool):
    if is_moving_to_right :
        current_player_image = PLAYER_IMAGE
    else :
...
def set_player_normal () :
    ...
    player.image = flip_image(is_moving_to_right)
    player.angle = 0
    # circle._surf = pygame.transform.scale(circle._surf, (100, 100))
